...
The original code uses `crypto.scrypt`, an asynchronous function that employs the common Node.js error-first callback pattern. This pattern involves passing a callback function as the last argument, which is invoked upon completion. The callback receives an error object as its first argument (or null if no error occurred) and the result as the second argument.

In my modified version, I utilized `util.promisify`, a built-in Node.js utility. This function converts a function using the error-first callback pattern into one that returns a Promise, allowing for more modern async/await syntax.

Note the type assertion as Buffer on the `derivedKey`. This explicitly informs TypeScript that, according to the `crypto.scrypt` documentation, a successful operation will resolve with a Buffer. Without this assertion, TypeScript might infer the type incorrectly (e.g. as unknown or a broader type) after the promisify transformation, potentially leading to type errors later when trying to call `toString("hex")`.
...
With `generateSalt()`, signing up different users with the same password will now result in different stored hashes because each password was combined with a unique salt before hashing.
...
2. **Store Session Data**: Store this session ID on the server-side (e.g. in a Redis database, as shown below) along with relevant user information (like user ID and role) and an expiration timestamp.
...
4. **Automatic Cookie Sending**: On subsequent requests to your application domain, the browser automatically includes the session cookie in the HTTP headers.
5. **Validate Session**: In your server-side code (often in middleware), extract the session ID from the incoming cookie. Look up this ID in your session store (Redis), validate that the session exists and hasn't expired, and retrieve the associated user data.
...
To address these needs efficiently and with type safety, we can create a flexible `getCurrentUser` function. This implementation leverages several techniques:
...
2. **Caching** (cache): Utilizes React's cache function (or a similar utility in Next.js) to memoize the result of the function within a *single server request/render cycle*. If `getCurrentUser` is called multiple times (e.g. in different components during the same page render) with the same arguments, the underlying logic (fetching from session/database) will only execute once, improving performance by avoiding redundant work.
...
- **protect private route**: only logged in user can access routes listed in the `privateRoute`
- **protect admin route**: only logged in user with "admin" role can access routes listed in the `adminRoute`
...
Let's compare the original approach (closer to the tutorial) and a revised version designed for potentially better efficiency and handling of overlapping routes.

Original Approach Explanation:

The original middleware function first calls a helper function `middlewareAuth` to perform the authorization checks.

1. `middlewareAuth` checks if the requested path is in the `privateRoute` list. If yes, it fetches the user session. If no user is found, it returns a redirect response to `/sign-in`.

2. If the route wasn't private or the user was found, it then checks if the path is in the `adminRoute` list. If yes, it fetches the user session again (potentially redundant if also a private route). It redirects to `/sign-in` if no user is found, or redirects to the homepage (/) if the user exists but lacks the 'admin' role.

3. If `middlewareAuth` completes without returning a redirect (meaning access is allowed or the route needs no protection), it returns `undefined`.

4. Back in the main middleware function, the nullish coalescing operator (??) is used: `(await middlewareAuth(request)) ?? NextResponse.next()`. If `middlewareAuth` returned a redirect NextResponse, that response is used. If it returned `null` or `undefined`, the expression evaluates to the right-hand side, `NextResponse.next()`, which allows the request to proceed normally.

5. Before returning the final response (either the redirect or NextResponse.next()), it calls `updateUserSessionExpiration`. This function reads the session ID from the incoming request cookies and updates its expiration time in the server-side session store (Redis). This implements the "session sliding".
...
1. It initializes a default `response = NextResponse.next()`.

2. It determines upfront if the requested pathname starts with any defined private or admin route prefixes using `pathname.startsWith(route)`. This is more flexible than an exact match (includes) if you have nested routes like `/admin/users` and `/admin/settings`.

3. If the route is identified as private or admin, it attempts to fetch the user session once. A `sessionChecked` flag is set. Error handling is added here in case session retrieval fails.

4. It then checks if the user is `null`. If they are (and the route required authentication), it immediately returns a redirect to `/sign-in`.
...
6. If the user exists, the session was checked `(sessionChecked && user !== null)`, and access is granted, then it attempts to update the session expiration via `updateUserSessionExpiration`. Error handling is added for this step too.

7. Finally, it returns the response (which will be `NextResponse.next()` unless a redirect was triggered earlier).
...
To allow users (presumably administrators) to change their own or others' roles, we can implement a Server Action.
...
Therefore, after successfully updating the database, we must also update the data associated with the user's current session ID in Redis. The `updateUserSessionData` function handles this:

1. It retrieves the current sessionId from the request cookies.
2. If a session ID exists, it overwrites the existing session data in Redis with the new user information (containing the updated role) using `redisClient.set`.
...
Modified `updateUserSessionExpiration` using EXPIRE:
...
2. If the ID exists, it uses `redisClient.expire()` to set a new TTL (SESSION_EXPIRATION_SECONDS) on the corresponding `session:<sessionId>` key in Redis.

3. It checks the result: expire returns 1 if the timeout was updated, and 0 if the key did not exist (e.g. the session already expired naturally between requests).

Crucially, this version does not need to interact with the response cookies (setCookie), because the session ID cookie itself doesn't change – only its corresponding server-side expiration in Redis is affected.
...
As you noted, Next.js middleware often defaults to running on the Edge runtime for performance benefits. However, the Edge runtime has limitations and doesn't support all Node.js APIs. A common issue arises with the built-in Node.js crypto module, particularly functions like `crypto.randomBytes`.

Even if your `middleware.ts` file doesn't directly import crypto, if it calls functions (like `createUserSession` from session.ts in this case) that do use Node.js-specific crypto functions, you might encounter runtime errors or warnings indicating incompatibility with the Edge environment (e.g. "crypto" is not compatible with the Edge Runtime).

At the time you were writing this (referencing Next.js 15.2 timeframe - Note: Experimental features and configurations can change; always consult current Next.js documentation), there were primarily two ways to resolve this:
...
1. Potentially installing a canary version of Next.js (`bun add next@canary` or `npm install next@canary`).
2. Enabling an experimental flag in next.config.js.
3. Specifying the runtime in the middleware's config export.

**Caution**: Relying on experimental flags is generally discouraged for production environments as they can change or be removed. This approach also sacrifices the potential performance benefits of the Edge runtime for middleware.
...
The preferred solution is to refactor the code causing the incompatibility to use APIs available in both Node.js and Edge environments. For cryptography, this usually means using the Web Crypto API, available via `globalThis.crypto`.

Specifically, you can replace `crypto.randomBytes` (Node.js specific) with `globalThis.crypto.getRandomValues` (Web Crypto standard) when generating the session ID in `createUserSession`.
...
2. **OAuth Account Linking**: We need a way to link a single user account in our system to potentially multiple OAuth identities (e.g. the same user might link both their GitHub and Google accounts). This typically involves creating a new table (e.g. OAuthAccount) with a foreign key relationship back to the main User table (a one-to-many relationship: one user can have many OAuth accounts). This new table would store the provider name (e.g. 'github'), the provider-specific user ID, access tokens (potentially encrypted), refresh tokens, etc.
...
Implementing the initial step of the OAuth flow – redirecting the user to the provider's authorization page – largely follows the tutorial. Pay close attention when setting up environment variables; typos are common pitfalls. One minor point from the video: ensure your base OAuth redirect URL (e.g. OAUTH_REDIRECT_URL_BASE in your .env) ends with a trailing slash (/) like http://localhost:3000/api/oauth/, as inconsistencies here can cause redirect URI mismatch errors later.

The `createAuthUrl` method within the OAuthClient class is responsible for constructing this provider-specific URL. While the function accepts cookies as an argument, its primary role in constructing the URL itself relies on the client's configuration (client ID, scopes, provider's auth endpoint, redirect URI). The cookies argument is used internally by helper functions (like `createState`, `createCodeVerifier`) to store security tokens (state, PKCE code verifier) that will be needed later in the flow for validation.
...
This `createAuthUrl` method is then typically called from a Server Action triggered by a button click in your sign-in form. The action creates an instance of the appropriate OAuthClient and redirects the user.
...
During development, successfully reaching the provider's consent page (e.g. Google's account chooser or GitHub's authorization request) is the first hurdle. Common errors at this stage usually stem from configuration mistakes:
...
- **Redirect URI Mismatch**: The redirect_uri sent in the URL parameters must exactly match one of the authorized redirect URIs configured in your application's settings on the provider's developer portal (e.g. [Discord Developer Portal](https://discord.com/developers/applications), [GitHub Developer settings](https://github.com/settings/developers), [Google Cloud Console](https://console.cloud.google.com/apis/credentials)). This is also where you obtain your Client ID and Client Secret.
...
- **Problem**: In the standard OAuth "Authorization Code" flow, the provider sends a temporary code back to your application (via browser redirect). Your backend then exchanges this code (plus your client secret) for an access token. If an attacker intercepts this code during the redirect (e.g. through malicious browser extensions or other vulnerabilities), they could potentially exchange it for an access token themselves (especially problematic for public clients like mobile apps that can't securely store a client secret).
...
- **Solution (state)**:
  1. **Client Creates State**: Before redirecting the user to the provider, your application generates a unique, unpredictable, and secret state value. This value is typically stored temporarily (e.g. in the user's session or a short-lived cookie).
  2. **State Sent**: Your app includes this state value as a parameter in the /authorize request URL sent to the provider.
  3. **State Returned**: The authorization provider must include the exact same state value when redirecting the user back to your callback URL (/api/oauth/callback?code=...&state=...).
  4. **Client Verifies**: When your callback handler receives the request, it compares the state value from the URL parameter with the value it stored earlier (e.g. retrieved from the cookie). If they don't match, or if no state was stored, the request is rejected, preventing the CSRF attack.
...
The key realization is that the very first step of the user-involved OAuth flow must involve the user's browser being redirected to the provider's domain (e.g. https://accounts.google.com/... or https://github.com/login/oauth/...). This is necessary for the provider to:
...
A common issue encountered with GitHub OAuth, as seen in the tutorial and experienced personally, is failing to retrieve the user's email address. The standard `/user` endpoint (https://api.github.com/user) might return null for the email field, even if the user has emails associated with their account. This often happens due to the user's GitHub email privacy settings (e.g. "Keep my email addresses private").

The `user:email` scope grants permission to read the user's email addresses, but you might need to fetch them from a different endpoint: https://api.github.com/user/emails.

### Implementation Strategy

My `userInfo.parser` for GitHub first attempts to get the email from the primary `/user` endpoint data. If it's null, it makes a second authenticated request to `/user/emails` to retrieve the list of emails, looks for the primary and verified one, and uses that.
...
***(Note: The full implementation involves additional Zod schemas (`githubBaseUserSchema`, `githubEmailSchema`) and more robust error handling, available in the repository.)***
...
Here are the standard Google OAuth 2.0 / OIDC endpoints I used (often found via LLM queries or specific OIDC discovery documents):
